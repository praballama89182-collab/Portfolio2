...
# Cycled blue palette for dynamic group counts (e.g. many brands)
BRAND_PALETTE = [
    "#2F88F5", "#1652A3", "#3A414B", "#226DCC", "#5B9BF7",
    "#0F3D75", "#6FA8F0", "#264D73", "#89A9CF", "#1E6FBF",
    "#4C7FD1", "#0B2A4A", "#7CB2F2", "#3D6FA8", "#193A5C",
]
...
def show_metrics_table(data: pd.DataFrame, first_col_label: str = "Group", height=None):
    ...
    styled = (
        d.style
        .format(fmt)
        .background_gradient(subset=pd.IndexSlice[non_total_idx, ["ACOS %"]], cmap="Blues")
        .background_gradient(subset=pd.IndexSlice[non_total_idx, ["ROAS"]], cmap="Blues")
...
def contribution_pie(labels, values, colors, title, value_prefix=""):
    fig = go.Figure(go.Pie(
        ...
        textfont=dict(size=12, color=COLOR_DARK),
    ...
    if trend_df.empty:
        ...
        with st.expander("Weekly breakdown (table)"):
            ...
            styled_weekly = (
                ...
                .background_gradient(subset=["ACOS %"], cmap="Blues")
            )
...
with tab_brand:
    ...
    if sku_col is None:
        ...
        st.caption("Click any column header to sort ascending/descending. Scroll within the table to see all brands.")

        brand_full_table = pd.concat([overall_table, overall_totals])
